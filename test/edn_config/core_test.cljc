...
(ns edn-config.core-test
  (:require
   [edn-config.core :refer [read-config reader
  ...
  #?(:clj (:import (edn_config.core Deferred))))
...
(deftest basic-test
  (let [config (read-config "test/edn_config/config.edn")]
    (is (= "Hello World!" (:greeting config))))
  (testing "Reading empty config returns nil"
    (is (= nil (read-config "test/edn_config/empty-config.edn")))))
...
(deftest hostname-test
  (is (= {:color "green" :weight 10}
         (read-config
          (source "test/edn_config/hosts.edn")
          {:profile :default :hostname "emerald"})))
  (is (= {:color "black" :weight nil}
         (read-config (source "test/edn_config/hosts.edn")
...
#?(:clj
   (deftest test-read
     (let [x [:foo :bar :baz]
           ...
           config (read-config "test/edn_config/config.edn")]
...
(deftest numeric-test
  ...
  #?@(:clj [(System/setProperty "FOO" "123")
           (let [config (read-config "test/edn_config/long_prop.edn")]
...
(deftest profile-test
  (let [config (read-config "test/edn_config/config.edn" {:profile :dev})]
    (is (= 8000 (:port config))))
  (let [config (read-config "test/edn_config/config.edn" {:profile :prod})]
    (is (= 80 (:port config)))))
...
(deftest default-reader-combo-test
  (let [config (read-config "test/edn_config/default-reader.edn")]
