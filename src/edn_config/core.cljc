...
(ns edn-config.core
  (:require
    [edn-config.alpha.core :refer
     [expand expand-scalar-repeatedly expand-case eval-tagged-literal
      reassemble kv-seq]]
    [edn-config.impl.walk :refer [postwalk]]
    #?@(:clj [[clojure.edn :as edn]
              [edn-config.impl.macro :as macro]]
     ...
     (let [fl
           (if (.isAbsolute (io/file include))
             ...
             (when-let [source-file
               ...
               (io/file (.getParent ^File source-file) include)))]
       (if (and fl (.exists fl))
         fl
         (StringReader. (pr-str {:edn-config/missing-include include}))))))
     ...
     (let [fl (if (path/isAbsolute include)
       ...
       (if (fs/existsSync fl)
         ...
         (source-logging-push-back-reader
           (pr-str {:edn-config/missing-include include}))))))
...
(defmethod eval-tagged-literal :default
  [tl opts env ks]
  (let [{:keys [:edn-config.core/incomplete?] :as expansion}
...
(defmethod eval-tagged-literal 'ref
  [tl opts env ks]
  (let [{:keys [:edn-config.core/incomplete? :edn-config.core/env :edn-config.core/value
                :edn-config.core/incomplete]
         :or {env env}
...
(defmethod eval-tagged-literal 'or
  ...
  (let [{:keys [:edn-config.core/incomplete? :edn-config.core/value] :as expansion}
    ...
    (if incomplete?
      ...
      (loop [[x & xs] value
             idx 0]
        (let [{:keys [:edn-config.core/incomplete? :edn-config.core/value]
               :as expansion}
...
(defn resolve-tagged-literals
  [wrapped-config opts]
  (let [{:keys [:edn-config.core/incomplete?
